	...
	$(function() {
		$('.subject').mouseover(function() {
			...
			$.ajax({
				// 요청
				type: 'get',
				url: 'bookAjax?isbn=' + isbn,
				daatType: 'json'
				
				// 응답
				success: function(result) {
					alert(result.book);
					//$('#resultView').html(result.isbn + result.title + result.catalog + result.author);
				}
			});
		});
	});	
...
<body>
	...
	<c:if test="${not empty user}">
		<h4>${user.name} 님 로그인 되었습니다. <a href="memberLogout">로그아웃</a></h4>
	</c:if>
	<c:choose>
		...
		<c:otherwise>
			<table id="bookTable">
				...
				<c:forEach items="${list}" var="book">
					<tr>
						<td>${book.isbn}</td>
						<td class="subject" id="${book.isbn}"><a href="bookView?isbn=${book.isbn}">${book.title}</a></td>
